...
def hook_fun(model, input, output):     		#定义hook
    ...
    #print(output.shape)
    out_FM = output
handle = model.avgpool.register_forward_hook(hook_fun) #注册一个前向hook
...
#=============================================================
zeros_FM = torch.zeros(512, 7, 7)  #torch.Size([512, 7, 7])
a_channel = torch.ones(7,7)  #torch.Size([7, 7])

weights = []
for k in range(512):
    zeros_FM2 = zeros_FM.clone()
    a_channe12 = a_channel.clone()
    zeros_FM2[k] = a_channe12
    x = zeros_FM2.reshape(-1) #torch.Size([25088])
    y = model.classifier(x)  #torch.Size([1000])
    w = y[pre_y].item()
    weights.append(w)
weights = torch.FloatTensor(weights).reshape(-1,1,1) #torch.Size([512, 1, 1])
