...
class Example4_1(nn.Module):
    def __init__(self):
        super().__init__()
        #下面创建两个卷积层：
        self.conv1 = nn.Conv2d(in_channels=1,out_channels=10,kernel_size=5)  # 【此处为本例核心代码，已省略，完整代码见教材P92；建议读者手工输入核心代码并进行调试，这样方能领会其含义】
    ...
    def forward(self,x):   	#x为输入进来的数据包，其形状为batch×1×28×28
        ...
        out = torch.relu(out)  # 【此处为本例核心代码，已省略，完整代码见教材P92；建议读者手工输入核心代码并进行调试，这样方能领会其含义】
...
for epoch in range(20): 		#迭代20代
    ...
    for i, (x, y) in enumerate(train_loader):
        ...
        if i%30 == 0:  #每计算30个包，做一次数据采样，以显示进度信息
            print(r'训练的代数：{} [当前代完成进度：{} / {} ({:.0f}%)]，当前损失函数值: {:.6f}'.format(epoch, i * len(x),len(train_loader.dataset), 100. * i / len(train_loader), loss.item()))
#-------------------------------------------
