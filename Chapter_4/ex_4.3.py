...
#=============================================
batch_size  = 20
# train_dir = '../../remote_GPU/data/catdog/training_set'  #训练集所在的目录，猫和狗的图像文件混在一起
# test_dir ='../../remote_GPU/data/catdog/test_set'        #测试集所在的目录，猫和狗的图像文件混在一起

# 获取当前脚本所在目录
current_dir = os.path.dirname(os.path.abspath(__file__))

# 设置数据路径 - 使用相对于脚本的路径
train_dir = os.path.join(current_dir, 'data/catdog/training_set')
test_dir = os.path.join(current_dir, 'data/catdog/test_set')
...
class Model_CatDog(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(64, 128, 5)  # 【此处为本例核心代码，已省略，完整代码见教材P113；建议读者手工输入核心代码并进行调试，这样方能领会其含义】
    ...
    def forward(self,x): # torch.Size([20, 224, 224, 3])
        ...
        out = self.conv1(x)
        out = nn.ReLU(inplace=True)(out)  # 【此处为本例核心代码，已省略，完整代码见教材P113；建议读者手工输入核心代码并进行调试，这样方能领会其含义】
