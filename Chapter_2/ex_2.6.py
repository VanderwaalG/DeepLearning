...
class Perceptron2(nn.Module): #必须继承类nn.Module
    def __init__(self):
        ...
        self.fc = nn.Linear(in_features=2,out_features=1,bias=True)  # 【此处为本例核心代码，已省略，完整代码见教材P51；建议读者手工输入核心代码并进行调试，这样方能领会其含义】
...
for ep in range(100):  				#迭代代数为100
    for (x, y) in zip(X, Y):
        ...
        loss = nn.BCELoss()(pre_y,y)  # 【此处为本例核心代码，已省略，完整代码见教材P51；建议读者手工输入核心代码并进行调试，这样方能领会其含义】
