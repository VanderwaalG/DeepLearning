...
#相当于将Q和K拼接（第一次）
        ...
        attention_weights = F.softmax(score_Q_K,dim=1)
        attention_QKV = torch.bmm(attention_weights.unsqueeze(0),en_outputs.unsqueeze(0))
        gru_input = torch.cat((embedded[0],attention_QKV[0]),1)
        gru_input = self.attention_com(gru_input).unsqueeze(0)
