...
class Encoder(nn.Module):
    ...
    def __init__(self, en_vocab_num, hidden_size):
        ...
        # 【此处为本例部分核心代码，已省略，完整代码见教材P218；建议读者手工输入核心代码并进行调试，这样方能领会其含义】
        self.hidden_size = hidden_size
        self.embedding = nn.Embedding(en_vocab_num,hidden_size)
...
class Decoder(nn.Module):
    def __init__(self, hidden_size, zh_vocab_num):#hidden_size表示词向量的长度，en_vocab_num表示中文词的数量
        super(Decoder, self).__init__()
        self.hidden_size = hidden_size
        # 【此处为本例部分核心代码，已省略，完整代码见教材P219；建议读者手工输入核心代码并进行调试，这样方能领会其含义】
        self.embedding = nn.Embedding(zh_vocab_num,hidden_size)
        self.gru = nn.GRU(hidden_size,hidden_size)
        self.fc = nn.Linear(hidden_size, zh_vocab_num)  #全连接网络，用于预测输出为各个词的概率
    def forward(self, x, h):
        x = self.embedding(x)
        x = F.relu(x)
        x = x.reshape(1, 1, -1)
